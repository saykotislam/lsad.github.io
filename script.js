...
function showResult() {
    ...
    // Check if the total score is 232 or above
    if (totalScore >= 232) {
        // Check if the second option is clicked 6 times from additionalQuizData
        const secondOptionCount = userAnswers.filter((answer, index) => {
            return additionalQuizData.findIndex(question => question.id === index + 1) !== -1 && answer === 1;
        }).length;

        // Check the selection from id:7 question
        const id7AnswerIndex = userAnswers[6]; // id:7 corresponds to index 6

        // Output based on conditions
        if (secondOptionCount >= 6 && id7AnswerIndex === 0) {
            quizContainer.innerHTML = "ডাক্তার";
            footer.style.display = 'none';
            return;
        }

        if (secondOptionCount >= 6 && id7AnswerIndex === 1) {
            quizContainer.innerHTML = "ইঞ্জিনিয়ার";
            footer.style.display = 'none';
            return;
        }
    }

    // Check if the total score is 203 or above
    if (totalScore >= 203) {
        // Check if the third or fourth option is clicked 6 times from additionalQuizData
        const thirdOptionCount = userAnswers.filter((answer, index) => {
            return additionalQuizData.findIndex(question => question.id === index + 1) !== -1 && answer === 2;
        }).length;
        ...
        // Output based on conditions
        if (thirdOptionCount >= 6 && id7AnswerIndex === 2) {
            quizContainer.innerHTML = "অধ্যাপক";
            footer.style.display = 'none';
            return;
        }

        if (thirdOptionCount >= 6 && id7AnswerIndex === 3) {
            quizContainer.innerHTML = "প্রশাসক";
            footer.style.display = 'none';
            return;
        }

        // Check selections from id:7 question for additional outputs
        for (let i = 4; i <= 16; i++) {
            if (thirdOptionCount >= 6 && id7AnswerIndex === i) {
                const profession = [
                    "শিল্পী",
                    "খেলোয়াড়",
                    "জার্নালিস্ট",
                ...
                quizContainer.innerHTML = profession[i - 4];
                footer.style.display = 'none';
                return;
            }
        }
    ...
    // Default output if none of the conditions are met
    quizContainer.innerHTML = "Do it again";
    footer.style.display = 'none';
}
